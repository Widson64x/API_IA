...
@roteador_admin.post(
    "/clientes/{cliente_id}/reset-key",
    status_code=status.HTTP_200_OK,
    summary="Redefinir / Gerar nova API Key",
    description="Gera uma nova API Key para um cliente existente e o reativa. A nova API Key é retornada apenas nesta resposta."
)
async def redefinir_chave_cliente(
    cliente_id: str,
    x_admin_key: str = Header(..., description="Chave de administracao da API")
) -> dict:
    """Redefine a API Key de um cliente e retorna a nova chave em texto plano.
    ...
    Returns:
        dict: Dados do cliente com a nova API Key.

    Raises:
        HTTPException (404): Se o cliente nao for encontrado.
    """
    from App.Core.GerenciadorApiKeys import redefinir_api_key

    _validar_admin_key(x_admin_key)

    resultado = redefinir_api_key(cliente_id)
...
@roteador_admin.delete(
    "/clientes/{cliente_id}/excluir",
    status_code=status.HTTP_200_OK,
    summary="Excluir cliente permanentemente",
    description="Remove permanentemente um cliente do banco de dados."
)
async def excluir_cliente_definitivo(
    cliente_id: str,
    x_admin_key: str = Header(..., description="Chave de administracao da API")
) -> dict:
    ...
    Returns:
        dict: Mensagem de confirmacao da exclusao.

    Raises:
        HTTPException (404): Se o cliente nao for encontrado.
    """
    from App.Core.GerenciadorApiKeys import excluir_cliente
    ...
    return {
        "mensagem": f"Cliente '{cliente_id}' excluido permanentemente.",
        "excluido": True
    }
